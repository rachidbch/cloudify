...
- **Tests**: new `tests/unit/vars.bats` (37 tests: helpers, resolver, reserved guard, R6 special chars/multi-line, precedence matrix, R1 local install path); full unit suite 342 green in `cloudai:cloudify`; pinned `remote-vars.bats` / `deployments.bats` / `package-api.bats` / `remote.bats` / `install-run-split.bats` green unmodified; both pinned integration files (`package-remote-vars`, `package-install-run-split`) green.
...
- **Docs**: README var sections corrected (five sources + precedence + secret references + reserved names); AGENTS.md "Configuration" bullet still stale (proposed, not edited — harness file).
