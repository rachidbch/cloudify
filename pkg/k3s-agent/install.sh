#!/usr/bin/env bash
# k3s-agent — install phase (ADR-008 split): k3s binary + airgap images +
# node prep + install guard. Run phase (config + service) is configure.sh.

# --- Install guard ---
if [[ -x /usr/local/bin/k3s ]] && [[ -d /var/lib/rancher/k3s/agent ]] && \
...
log_info "k3s-agent binary installed (${K3S_VERSION}). Run configure to join a cluster."
