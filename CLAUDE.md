...
- **Credential management**: Credentials are stored in `~/.config/cloudify/credentials` (XDG-compliant, `chmod 600`). `cloudify_credentials_load()` is called at startup in `cloudify_init_paths()` and sources the file without overwriting existing env vars. `cloudify_credentials_save [section]` persists a specific section (remote, github, gitlab, restic). `cloudify_credentials_migrate()` handles one-time migration from legacy locations (`~/cloudify/.credentials`, `/dev/shm/cloudify_credentials`). The `cloudify credentials` subcommand supports section-based prompting: `cloudify credentials remote` prompts only for `CLOUDIFY_REMOTE_USER`/`CLOUDIFY_REMOTE_PWD`. `launch`/`delete` commands require no credentials — they delegate to ivps.
