...
function cloudify_remote_sync() {
    ...
    if [[ "$host" == "localhost" ]]; then
        ...
        cloudify_remote_payload=$(envsubst \
            '$CLOUDIFY_DISABLE_COLORS $DEBUG $CLOUDIFY_LOG_LEVEL $CLOUDIFY_NO_DEFAULTS $CLOUDIFY_FORCE_UPDATE $CLOUDIFY_UPDATE_DELAY $CLOUDIFY_REMOTE_USER $CLOUDIFY_REMOTE_PWD $CLOUDIFY_GITHUBUSER $CLOUDIFY_GITHUBPWD $CLOUDIFY_GITLABUSER $CLOUDIFY_GITLABPWD $CLOUDIFY_RCLONE_REMOTE $CLOUDIFY_RCLONE_REMOTE_REGION $CLOUDIFY_RCLONE_REMOTE_ENDPOINT $CLOUDIFY_RCLONE_REMOTE_ACCESSKEYID $CLOUDIFY_RCLONE_REMOTE_SECRETACCESSKEY $RESTIC_PASSWORD $CLOUDIFY_BOOTSTRAP_URL $CLOUDIFY_SIGNAL_PORT $CLOUDIFY_OPENWEBUI_PORT $WEBUI_ADMIN_EMAIL $WEBUI_ADMIN_PASSWORD' \
