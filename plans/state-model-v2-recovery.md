...
The per-name field set is not part of the flat context; it is roadmapped (see "Dispatch context as a JSON contract" in `ROADMAP.md`).
...
The checklist was reconciled against HEAD 6857081 after the Phase 2 repair (2026-09-14). Twelve lines were already true and are ticked with the code and test that prove them; the surviving lines carry the concrete finding that makes them false. Ticks are proof, not promise: each names file:line and its test.

### CRITICAL GATE before any `lib/` edit (R2.0)

- [x] Reuse the forwarding description artifact (R1.0), or extend it, to cover whatever Phase 3 code the audit will change. (Extended by `tmp/state-model-v2-phase3-description.md`, 813 lines, read-only subagent, 2026-09-14: the six `.remote-vars` parser copies and their accepted shapes, the seventeen flat `KEY: value` loops, the declared-names gate and its three-field candidate format, the manifest encode/read/validate stack, canonical versus glob runbook discovery, the front-matter readers, the manifest lifecycle, the dead aliases and the `cloudify deployments` listing bug.)
- [x] State why each Phase 3 fix cannot break the forwarding path, the shadows or any recipe, and obtain Rachid's consent recorded in `LOGS.md`. (`tmp/state-model-v2-phase3-non-breakage.md`; consent 2026-09-14 in `LOGS.md`, scope: `lib/state.sh`, `lib/runbooks.sh`, `lib/vars.sh`, `lib/context.sh`, `lib/registry.sh`, `lib/deployments.sh`, `lib/utils.sh`, `cloudify` (alias and one comment), `README.md`, the coupled `tests/unit/*`. Decisions: `jq` becomes a hard prerequisite of the state path; the `cloudify deployments` listing bug is fixed here.)

### Manifest and state-root audit (R2.1)

- [x] Prove every `manifest.json` is actual JSON that validates against `schemas/v1/deployment-manifest.schema.json` before atomic replacement. (Done 2026-09-14: the manifest is rendered with `jq -n` and validated with the schema checker before the `mv`, and `jq` is a hard prerequisite, so the check can no longer be skipped. `tests/unit/state.bats`, "the schema checker is the one validator and jq is required".)
- [x] Prove component validation happens before any path creation. (`cloudify_state_deployment_dir` validates all three components before printing (`lib/state.sh:150-155`); `cloudify_manifest_write` resolves the directory before `cloudify_state_ensure_dir` (`:471-475`); `tests/unit/state.bats:68`, `:245`.)
- [x] Prove manifest writes take one deployment lock and no host lock is held at the same time. (`_cloudify_state_run_locked` is the only lock in the repository and its sole caller is `cloudify_manifest_write` (`lib/state.sh:106-121`, `:476`); `tests/unit/state.bats:182`.)
- [x] Prove lifecycle ordering: `applying` before mutation, `active` after install plus verify, `degraded` after observed failure. (`applying` and `degraded` already held; the `active` rule is fixed 2026-09-14 to require both install and verify, so an install-only run keeps its recorded status. `tests/unit/runbooks.bats`, "an install-only run never becomes active".)
- [x] Prove a killed writer remains discoverable without fabricating a run record. (No state run writer exists; a kill leaves `status: applying`, exposed by `cloudify_manifest_describe` (`lib/state.sh:545-547`); `tests/unit/runbooks.bats:899-920` asserts no `runs/` directory appears.)
- [x] Keep applied package values out of the manifest. (The renderer emits exactly the schema fields (`lib/state.sh:301-350`), the validator enforces the exact field set (`:409-413`), the schema is `additionalProperties: false`; `tests/unit/state.bats:125-165`.)
- [x] Create a manifest only from an application run whose current commit is proved; old desired-input migration never fabricates a manifest or application commit. (The only creator is `_cloudify_deployment_manifest_prepare` from `cloudify_deployment_run` (`lib/runbooks.sh:835-877`, `:1273-1275`), and it dies on a dirty or unidentified tree unless `CLOUDIFY_DEVELOPMENT_OVERRIDE=1` (`:790-804`); `cloudify_deployment_migrate` writes only the desired-input store (`lib/deployments.sh:282-347`).)

### Application and runbook audit (R2.2)

- [x] Keep only `runbooks/<application>/<flavor>/runbook.md` discovery. (Done 2026-09-14: `cloudify_runbook_find` filters every candidate through `cloudify_runbook_identity` and only searches `runbook.md`. `tests/unit/runbooks.bats`, "a Markdown file that is not runbook.md is invisible".)
...
- [x] Prove target bindings persist and ordinary reruns cannot silently rebind them. (Recorded bindings replay when the CLI did not supply the slot, and a differing `--target` dies without `--migrate-targets` (`lib/runbooks.sh:1211-1225`, `:859-862`); `tests/unit/runbooks.bats:949`, `:964`.)
- [x] Prove direct package commands bypass application manifests as specified. (No manifest writer has a caller outside `lib/runbooks.sh` and `lib/state.sh`; a direct package command goes through `_cloudify_execute_package_action` into context plus registry observation only (`cloudify:241`).)
- [x] Delete dead aliases, duplicate parsers, duplicate path builders and stale compatibility language. (Done 2026-09-14: `add_in_hosts`, the `cloudify_vars_set/delete/list/show` and `_cloudify_deployment_read_vars` aliases, the undocumented `migration` verb alias, the stale `_cloudify_note_bg` comment and the stale README value-source list are gone; their test assertions moved to the real functions.)
- [x] Reduce `lib/runbooks.sh`, `lib/state.sh`, `lib/context.sh`, `lib/deployments.sh` and `lib/vars.sh` where the same fact is parsed or formatted more than once; inline each duplicate into one existing parser rather than adding another abstraction layer. (Done 2026-09-14: one pure `.remote-vars` enumerator, `cloudify_vars_declared_names`, behind all six copies with `cloudify_vars_pkg_read` keeping the only claim and provenance write; one front-matter key reader behind the deployment id, the meta parse and the inputs/map reads; one `--target` predicate behind the four argv checks; the bare-run phase set spelled once; one JSON encoder. The two flat readers on the resolution ladder stay separate on purpose - different key case, quote and empty-value rules, and the claim set is upstream of the envsubst allow-list - as do the four legacy single-ID readers in `cloudify_deployment_migrate`, which the read surface phase (Phase 8) deletes. Reason recorded in `tmp/state-model-v2-phase3-non-breakage.md` section 9.)
- [x] Replace `lib/state.sh`'s hand-rolled JSON encoder, field reader and bindings parser (`_cloudify_manifest_render`, `_cloudify_manifest_field_file`, `_cloudify_manifest_parse_bindings`) and its optional-jq shell fallback with `jq`, so the JSON artifacts (the deployment manifest, run records, package state) share one encoder and one validator. (Done 2026-09-14: the manifest is rendered with `jq -n`, read with `jq -r`, and validated by `schemas/v1/lib/schema-check.jq`; the shell fallback and the duplicate field/enum/identity checks are deleted. `jq` is a hard prerequisite of the state path with a loud pre-mutation failure and a README prerequisite line. `tests/unit/state.bats`.)
- [x] Replace the all-zeros `_CLOUDIFY_MANIFEST_NULL_COMMIT` sentinel with real `null` in the manifest and run records, now that the schemas allow it, and make the shell manifest validator accept null only with `development_override: true`. (Done 2026-09-14: the sentinel is gone, the renderer emits JSON `null`, and the schema's `allOf` rule is the gate. `tests/unit/state.bats`, "an unproved commit is real null, only with the development override"; `tests/unit/runbooks.bats`, "an unidentified tree records a real null commit with the override". Run records have no writer until Phase 6.)
- [x] Fix `cloudify deployments`: it printed every first-level directory under `CLOUDIFY_DEPLOYMENTS_DIR`, which is now application names, because that root holds both the nested `<app>/<flavor>/<name>` desired-input tree and the legacy single-ID run directories. (Done 2026-09-14: the listing is manifests only, which is what `REDESIGN.md` specifies. `tests/unit/deployments.bats`, "never prints a bare directory as a deployment".)
