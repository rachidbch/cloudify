...
### 2026-09-10 - branch 7 T2: registry storage primitives

- New `lib/registry.sh` (guard `_CLOUDIFY_REGISTRY_LOADED`) + one router source line. Storage primitives only: no install wiring, no record schema, no change to the walker, payload template, envsubst allow-list, shadows or recipes (I7-1, I7-2, I7-7 hold).
- Bucket: `$(ivps node path <node>)[/<instance>]` when resolvable, else the plain-host bucket `${CLOUDIFY_CREDENTIALS_DIR:-$HOME/.config/cloudify}/registry/hosts/<ssh_host>[/<instance>]` (resolved at call time; `ivps` absent or failing never aborts the caller; `local` stays on the fallback until the upstream ivps fix). Record path inside a bucket: `<bucket>/deployments/<id>/pkgs/<pkg>/config.yaml`.
- API: `cloudify_registry_file` (pure, prints the path; `/`, `.`, `..` rejected in every component, empty deployment/pkg rejected), `_put` (record on stdin, umask 077 `mkdir -p` + `chmod 700` parent, `mktemp` in-dir + `mv` = atomic replace, file 0600), `_get` (rc 1 + no output when absent), `_delete` (removes via `trash-put` with an `rm -f` fallback, prunes the now-empty `<pkg>` and `pkgs` dirs, spares `deployments/<id>` and the ivps-owned bucket root, rc 0 when absent), `_list` (`<deployment> <node> <instance> [<ssh_host>]`, one line per recorded package, empty when none).
- Spec ambiguities resolved and documented in the module header: `list` takes an optional `ssh_host` (only the fallback bucket needs it; unaddressable bucket lists nothing); an unaddressable bucket (no node dir + no ssh host) dies for `file`/`put`/`get`/`delete`; pruning stops below `deployments/<id>` (`deployments/<id>` survives, per the plan's "prunes ... never above the deployment dir" + "spares the deployment dir").
- Tests: new `tests/unit/registry.bats` (27 cases, rubric-led) — red-first run recorded (all 27 failed on the missing module in the container); node/instance paths, three fallback triggers (unresolvable node, empty node, absent ivps), `CLOUDIFY_CREDENTIALS_DIR`-vs-`HOME` fallback root, 0700/0600 perms under a 0755 base, put/get round-trip, atomic replace (new inode + no temp litter + perms kept), idempotent put, delete prune/spare/no-over-prune/absent-rc, list bucket isolation, unsafe-component rejection, guard.
- Evidence: `task lint` rc 0; plain driver `~/tmp/t2/driver.sh` all green on bash 5.x; `task test-unit` 452/452 rc 0 on final HEAD (was 425 after T1; run once). Router smoke in `cloudai:cloudify`: `cloudify help` rc 0 and `--on cloudify:` still dies on the T1 vps-required path (container has no ivps) — no regression from the added source.
- Unproven: no write against a real `ivps node path` directory (unit stub only); the fallback retire awaits the upstream ivps `local` node fix. On branch `feat/state-registry`, not merged.

### 2026-09-10 - branch 7 T3: registry write (record schema + dispatch plumbing)

- `lib/registry.sh` gains the writer: `cloudify_registry_record_build <action> <deployment> <node> <instance> <ssh_host> <pkg>` (pure — reads the existing record to merge timestamps + version, prints), `cloudify_registry_record_apply` (build | `cloudify_registry_put`, atomic 0700/0600), and `_cloudify_registry_record_bg <pid>` (the wait-loop hook).
- Schema (flat, first line a `#` comment): `status`, `installed_at`, `configured_at`, `removed_at`, `deployment`, `node`, `instance`, `package`, `version`, then one `var.<NAME>` per declared name. Empty fields print as `key:`. `var.<NAME>` is the RAW value from the first providing source env > deployment > package > global (a stored `@base64:`/`@backend:` reference stays a reference); a multi-line raw value is stored `@base64:` (var-store encoding); undeclared/unresolved names are skipped; `version` stays empty (no source).
- Merge: install/configure/uninstall set only their own status + timestamp; earlier timestamps and version survive (`install -> configure` keeps `installed_at`).
- **Uninstall keeps the record** (`status: removed`, `removed_at`) — the registry is observation, so teardown is a timestamp, not a delete. Supersedes the earlier T4 sentence; T4 is now `deployment delete` only (plan text corrected).
- Router plumbing (operator-side): `main()` declares `_CLOUDIFY_BG_ACTION/_PKGS/_TARGET` (assoc, parallel to `_CLOUDIFY_BG_HOSTS`); `_cloudify_note_bg` records action + packages + the resolver triple for every backgrounded dispatch; local uses `local//localhost`, remote reuses the triple T1 stored in `_CLOUDIFY_TARGETS` (no extra `ivps` call). The final wait loop calls `_cloudify_registry_record_bg` only for a successful `wait` and only for install/configure/uninstall; `verify` writes nothing; an unset `CLOUDIFY_DEPLOYMENT` skips with a debug line; a write failure warns and never aborts (I7-2/I7-8). No walker/payload/envsubst/shadow/recipe change.
- Tests: new `tests/unit/registry-write.bats` 12 cases (rubric-led: install schema + UTC timestamp, unknown action, perms, configure/uninstall merge, raw precedence, reference stays raw, `@base64:` round-trip, declaration order/no leak, unset-deployment gate, per-package write, verify no-op) + 1 real-router case in `tests/unit/shell-router.bats` (stub ssh + ivps; the record lands under the resolver's node dir with `node: cloudai`, `instance: cloudify`). `task lint` + `bash -n` rc 0; driver `~/tmp/t3/driver.sh` green; full unit suite 465/465 rc 0 (`1..465`, once on final HEAD).
- E2E smoke (real ssh, throwaway id): `CLOUDIFY_DEPLOYMENT=t3-smoke-1789060828 bash cloudify --on cloudai:cloudify install bats-test` rc 0 -> `~/.config/ivps/nodes/cloudai/cloudify/deployments/<id>/pkgs/bats-test/config.yaml` (700 dir / 600 file, `status: installed`, `node: cloudai`, `instance: cloudify`, UTC `installed_at`); a re-run left exactly one file and a valid schema (only `installed_at` moved). Throwaway record dir trashed afterwards; no fallback bucket created.
- Unproven: no E2E for configure/uninstall merge (bats-test has no configure/uninstall leg) — unit-only there; local-path records still use the fallback bucket until ivps exposes `local`. On branch `feat/state-registry`, not merged.

### 2026-09-10 - branch 7 T4 + T5: registry cleanup on `deployment delete` + control-channel reserved names

- `lib/registry.sh` gains `cloudify_registry_delete_deployment <id>` (+ `_cloudify_registry_delete_record_tree`). Records live outside the deployment store, so the sweep enumerates candidate bucket roots by glob (never shelling out to ivps): every dir under `${IVPS_CONFIG_DIR:-${XDG_CONFIG_HOME:-$HOME/.config}/ivps}/nodes` and under `${CLOUDIFY_CREDENTIALS_DIR:-$HOME/.config/cloudify}/registry/hosts`. In each root it removes `<root>/deployments/<id>` (node target) and `<root>/*/deployments/<id>` (instance target) only when the dir holds a `pkgs/` subdir, so a same-named unrelated directory is never touched; `trash-put` with an `rm -rf` fallback; every removal is printed (`Registry: removed record dir <path>`); rc 0 when nothing matches. Stated ambiguity resolved: ivps itself resolves `CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/ivps"`, so the sweep honours `IVPS_CONFIG_DIR` (test override) then replicates the XDG default.
- `cloudify_deployment_delete` calls it after trashing the store dir, and also for an id whose store dir is already gone (orphan records). The call is `declare -F`-guarded so `lib/deployments.sh` stays usable when sourced alone (tests). Router usage line updated. An uninstall still KEEPS the record (`status: removed` + `removed_at`, T3); `cloudify_registry_delete` remains the explicit per-slice erase.
- `lib/vars.sh` deny-list gains `CLOUDIFY_FORCE`, `CLOUDIFY_NO_VERIFY`, `CLOUDIFY_DEPLOYMENT`, `CLOUDIFY_NODE`, `CLOUDIFY_INSTANCE`: a file store can no longer flip the dispatch control channel or the target id. Warn+skip behaviour and message unchanged; no other walker change. (`CLOUDIFY_FORCE`/`CLOUDIFY_NO_VERIFY` still travel operator-env -> payload via the envsubst allow-list; only file stores are gated.)
- Tests: new `tests/unit/registry-delete.bats` 14 cases (red first: 13 failed before the lib change, 1 spurious pass on the undefined-command check) + 1 case in the existing reserved-name section of `tests/unit/vars.bats` (existing assertions untouched). L0 `bash -n` + `task lint` rc 0; focused in `cloudai:cloudify`: `registry.bats + registry-delete.bats + vars.bats` 82/82, `deployments.bats + registry-write.bats + shell-router.bats` 57/57; full `task test-unit` 480/480 rc 0 (`1..480`, once on the final tree: 465 + 15).
- E2E smoke (real ssh, two throwaway deployment ids, `bats-test` on `cloudai:cloudify`): both installs wrote `~/.config/ivps/nodes/cloudai/cloudify/deployments/<id>/pkgs/bats-test/config.yaml`; `cloudify deployment delete <id>` removed that id's record dir + store dir and printed the removal, while the sibling id's record, `node.json` and the bucket survived. Both throwaway ids trashed afterwards.
- Unproven: the `rm -rf` fallback for a missing `trash-cli` is not exercised (tests take whichever is installed); fallback-bucket cleanup is unit-only, no real plain-host E2E. On branch `feat/state-registry`, not merged.

### 2026-09-10 - branch 7 T8 + T9: ADR-020 + dev-skill Security sections

- ADR-020 (accepted): deployment state is observation (one record per deployment/target/package under the ivps node dir, 700/600, atomic, fallback bucket); replay is a playable runbook (the plan) + values (deployment store + run snapshot), never through the precedence ladder; targets are named slots bound at run time via the `--on` grammar; secrets are references where the source was a reference, else raw, never git. Supersedes ADR-011 points 3, 6 and 7; ADR-011's status line annotated, body untouched.
- Dev skills: `cloudify-dev` gets a Security section (registry local/0600/never git; runbooks carry structure + names only; control-name deny-list); `cloudify-pkg-dev` gets one (recipe plaintext only in its own 0600 state files; cloudify state carries a reference). No code change.

### 2026-09-10 - branch 7 T6a: runbook parser + target binding + preflight + `deployment run --dry-run`

- New `lib/runbooks.sh` (guard `_CLOUDIFY_RUNBOOKS_LOADED`), sourced by the router; `cloudify deployment run <id> [--runbook <path>] [--target name=addr]... [--from <id>] [--dry-run] [--yes]` added to the `deployment` verb. Additive and operator-side: no walker/payload/envsubst/shadow/recipe change.
- Contract: `cloudify_runbook_parse` -> `type\tid\ttarget\tpkg\tbody-b64`; `cloudify_runbook_meta` -> `deployment\ttargets-csv`; `cloudify_runbook_find <id> [root]` (default `${CLOUDIFY_DIR}/runbooks`, resolved like `pkg/`); `cloudify_runbook_bind_targets` -> `name\tnode\tinstance\tssh_host`; `cloudify_runbook_preflight`. Step fences `bash step=<type> [target=] [pkg=] [id=]`, types `launch|install|configure|verify|uninstall|human-gate`. Parse dies (path + line) on unknown type/attribute, missing target, missing pkg for install/configure/verify/uninstall, undeclared target, duplicate id; auto id = step position (`%02d`); body base64 so multi-line shell survives.
- Bindings: `--target` first, else deployment-store `TARGET_<NAME>` (uppercased); each validated via `_cloudify_target_resolve`; unbound dies listing every needed binding; a binding for an undeclared target dies. Preflight exports the runbook's deployment so `_cloudify_vars_source_of` consults its store, then dies listing every `pkg: NAME` that only the recipe default provides. `--dry-run` prints deployment/targets/steps and exits 0; without it the plan prints then dies "step execution lands in T6b" (nothing runs).
- Spec ambiguities resolved (recorded in the plan T6a note): non-step fences ignored; unknown attribute fails closed; front-matter duplicates deduped; `--runbook` must declare the same `deployment:` as `<id>`; `--from` validated but unused; `--yes` accepted/ignored; a referenced pkg need not exist for T6a.
- Tests: new `tests/unit/runbooks.bats` 22 cases (fixtures `tests/fixtures/runbooks/valid.md`, `guest-only.md`) covering parse + every rejection + discovery (match/none/two) + binding (override/store/unbound/undeclared) + preflight (missing lists, provided passes) + the real router `deployment run --dry-run` and the T6b refusal. L0 `bash -n` + `task lint` rc 0; driver `~/tmp/t6a/driver.sh` green; focused `runbooks.bats + shell-router.bats + deployments.bats` 67/67 in `cloudai:cloudify`; full `task test-unit` 502/502 rc 0 (`1..502`, once on final HEAD; 480 + 22).
- Unproven: nothing executes in T6a (no real-ssh run); the non-dry-run path is a deliberate `die`; `--from`/`--yes` semantics and step outputs land in T6b; no repo runbook uses the typed format yet (T7 authors it). On branch `feat/deployment-run`, not merged.

### 2026-09-10 - branch 7 T6b: runbook execution + step outputs + human gate + run snapshot

- `lib/runbooks.sh` gains `cloudify_runbook_execute <path> [--target name=addr]... [--from <id>] [--yes]`; the `deployment run` branch now dispatches it after printing the plan (additive; no walker/payload/envsubst/shadow/recipe change). `--from` and `--yes` now take effect; `--dry-run` stays plan-only.
- Contract: preflight first (targets + required vars); run-wide exports `CLOUDIFY_DEPLOYMENT`, `TARGET_<NAME>` (the resolved address `node:instance`/`node`/plain ssh host, so a body can pass it straight to `--on`), `CLOUDIFY_OUTPUTS_FILE`; per step `STEP_ID/STEP_TYPE/STEP_TARGET/STEP_PKG`. Steps run in document order (from `--from`), body via `bash -c` in the run env, stdout/stderr streamed; the first non-zero stops the run and is reported with the step id. A step appends `name=value` to the outputs file; the engine exports each as `OUT_<name>` for later steps and records it. `human-gate` prints its body and requires a TTY confirmation (`--yes` skips; no TTY + no `--yes` fails).
- Run snapshot `${CLOUDIFY_DEPLOYMENTS_DIR}/<id>/runs/<utc>.yaml` (0600, atomic mktemp+mv, 0700 dir), written on success and failure: `status: succeeded|failed`, `started_at`/`finished_at`, `runbook`, `target.<name>`, `value.<NAME>` (raw deployment-store lines), `output.<name>`. Temp outputs file removed. The snapshot is not a registry record (separate tree).
- Ambiguities resolved: the design's `cloudify_vars_deployment_file <id>` does not exist - used `_cloudify_deployment_config <id>`; output values stored verbatim; a failing step's outputs are still recorded; a `human-gate` refusal writes `status: failed` then dies.
- Tests: new `tests/unit/runbook-exec.bats` 6 cases (order + OUT_<name> passthrough + snapshot value/output + 0600; first-failure stop; human-gate `--yes`/no-TTY; `--from`; unknown `--from`; snapshot vs registry). `runbooks.bats`'s non-dry-run case now asserts real execution (28 runbook cases). L0 `bash -n` + `task lint` rc 0; driver `~/tmp/t6b/driver.sh` green; focused `runbook-exec + runbooks + deployments + shell-router` 73/73 in `cloudai:cloudify`; full `task test-unit` 508/508 rc 0 (`1..508`, once on final HEAD; 480 + 28).
- E2E (real ssh): throwaway runbook `t6b-smoke` (step `cloudify --on cloudai:cloudify verify bats-test` + a step appending an output) run via `cloudify deployment run ... --target guest=cloudai:cloudify --yes`; plan + step output + snapshot (`status: succeeded`, `target.guest: cloudai:cloudify`, `output.stamp: ...`, 600) shown; deployment deleted after (snapshot removed with it).
- Unproven: the interactive human-gate "yes" path (only `--yes` and the no-TTY refusal tested); a `human-gate` mid-run followed by more steps over a real TTY; replay (T6c). On branch `feat/deployment-run`, not merged.

### 2026-09-10 - branch 7 T6c: `deployment replay` (re-run a recorded run) + T6 complete

- `lib/runbooks.sh` gains `cloudify_deployment_replay <id> [--at <run>] [--runbook <path>] [--target name=addr]... [--from <id>] [--dry-run] [--yes]`; the router gains the `deployment replay` dispatch and usage line (additive; no walker/payload/envsubst/shadow/recipe change).
- Snapshot selection: `--at` matches an existing path, a basename or a timestamp prefix under `${CLOUDIFY_DEPLOYMENTS_DIR}/<id>/runs/`; the default is the most recently written snapshot (mtime). None or several matches die listing them - never a silent pick.
- Seeding (before anything executes): each `target.<name>` becomes a `--target` binding (a command-line `--target` still wins); each `value.<NAME>` is resolved with `_cloudify_resolve_var_value` (a stored `@base64:`/`@backend:` reference must be decoded here or the step would receive the literal) and exported, so the ladder's strongest source - the caller env - forwards exactly the recorded values. The runbook defaults to the snapshot's `runbook:` path (`--runbook` overrides). Then the same engine as `deployment run` runs: preflight + steps + a new run snapshot. The plan prints target addresses and value NAMES only; a resolved value is never printed and never passed on a command line.
- New engine input `CLOUDIFY_RUNBOOK_SNAPSHOT_VALUES=<snapshot>`: the new snapshot records that file's `value.*` lines instead of the deployment store's current state, so a replay stays replayable after the store changes (unset = unchanged behavior).
- Fix surfaced by the T6c E2E (T6b artifact): `<utc>.yaml` was silently overwritten when a run and its replay landed in the same second - a chained run/replay does. The engine now suffixes `-2`, `-3`, ... until the name is free and the selector's "newest" is mtime-based (name order cannot order a same-second pair); `runbook-exec.bats`'s `| tail -1` newest heuristic became `_newest_snapshot` (mtime).
- Fail closed: a framework-owned (`value.CLOUDIFY_FORCE`) or malformed (`value.9BAD`) name dies before any step; a file without a `runbook:` line is not a snapshot; a snapshot whose runbook is gone dies pointing at `--runbook`.
- Tests: new `tests/unit/runbook-replay.bats` 11 cases (seed + new snapshot, resolved reference, target bindings + `--target` override, snapshot runbook default/override/missing, `--at` path/basename/prefix + newest default, none/no-match/ambiguous/not-a-snapshot, `--dry-run` names-only, framework-owned + malformed names, same-second no-overwrite, real router dispatch + usage). L0 `bash -n` + `task lint` rc 0; driver `~/tmp/t6c/driver.sh` 18/18; focused `runbook-replay + runbook-exec + runbooks + deployments + shell-router + vars + registry + registry-write` 163/163 in `cloudai:cloudify`; full `task test-unit` 519/519 rc 0 (`1..519`, once on final code HEAD; 508 + 11).
...
### 2026-09-11 - state model redesign (design talk, docs only)

- Two documents written and committed (`a5d5561 deepseek flash redesign`): `GLOSSARY.md` (the concept set, plain-word definitions) and `REDESIGN.md` (what changes in cloudify and ivps).
- Root defect found: for one run, the same values were computed twice by two different walks and written to two places nobody compared, the snapshot's `value.*` from the deployment store only, and the registry record's `var.*` from environment, deployment, package and global. That is the DRY failure the talk started from.
- Decided (ADR-021): an application is the runbook at `runbooks/<app>/<flavor>/runbook.md`; a deployment is the outcome of running one, named at run time, with no entity and no per-deployment defaults file; names and defaults live in cloudify, applied values live in the state, and the walk is step environment, then package state record, then defaults; one value two packages must agree on is one name; the state describes now and the past lives in the event log (one file per event, written by both tools); legs are phases, a bare run is install then verify, and reconfigure is explicit and seeded from the state record; a run state record replaces the per-run snapshot; rebuilding from the log is the exceptional repair path; the read surface is four commands.
...
### 2026-09-14 - merged to master: state model v2 Phase 2 repair and Phase 3 audit

- `state-model-v2-phase1` merged to `master` (05d6515, `--no-ff`), bringing the repaired value resolution (one dispatch context feeding payload, registry record and run snapshot; one JSON manifest encoder and validator) and the audited Phase 3 foundations. `master` had been at the Phase 1 baseline (`ee0ed22`).
- Merge gate was the branch's own Phase 3 gate: lint rc 0, full unit suite 657 ok / 0 not ok, two-host E2E 4 scenarios green with the 4 later-phase scenarios skipped by name and both hosts torn down, SPEC review PASS (claude) and Technical review PASS (codex) on fresh contexts.
- Harness docs corrected in the same push: the runbook path (`runbooks/<app>/<flavor>/runbook.md`, the only discoverable shape), the six-source value ladder ending at `deployments/<app>/<flavor>/<name>/values.yaml`, and `task lint` described as shellcheck on this host (AGENTS.md, CLAUDE.md, README.md).
- Branch name note for the trail: `state-model-v2-phase1` was cut for Phase 1 of the state-model-v2 plan and then carried Phases 2 and 3; it stays the working line for Phase 4 onward.

### 2026-09-14 - one vocabulary: `--on` takes a target host

- The value typed after `--on` was called three names (`token` in `lib/targets.sh` and its messages, `target reference` in GLOSSARY.md, `--on <target>` in AGENTS.md and REDESIGN.md). Standardized on **target host** per Rachid's decision.
- Gate first: `tmp/target-host-vocabulary-description.md` (237 lines, read-only subagent) classified every occurrence - code identifier, internal comment, user-visible string, doc prose, or an unrelated sense (credential/k3s/lexer `token`, the target-slot concept, the resolved (node, instance, ssh_host) triple) - and pinned the contracts; `tmp/target-host-vocabulary-non-breakage.md` states why the rename cannot break the resolver or its consumers. One description claim was disproved before it could mislead: `cloudify.sh` is a symlink to `cloudify` (git mode 120000), not a byte-copy.
- Renamed: the resolver's local variable (`token` -> `target_host`), the ten `Target '<value>'` message prefixes -> `Target host '<value>'`, one registry message, the three comments, the router's usage title (`target host grammar:`) and `No target host found.`, GLOSSARY's `## target reference` -> `## target host`, AGENTS.md's two `--on` lines, README's `Targets (--on)` paragraph, REDESIGN's two occurrences, and the schema's `address` description. Unchanged by construction: every credential/k3s/lexer `token`, `## target slot`, the machine field names (`target:`, `address`/`node`/`instance`/`ssh_host`, the bindings line), the ADR bodies and `schemas/v1/identity.md` (frozen).
- Ladder: lint rc 0, `bash schemas/v1/validate.sh` rc 0 (67 fixtures), targets suite green with a new assertion pinning the prefix, full unit suite 657 ok / 0 not ok (`results/target-host/report.tap`), two-host E2E 4 scenarios green with the 4 later-phase scenarios skipped by name and both hosts torn down (`results/target-host-e2e2/report.tap`).
- Finding recorded for a separate gated fix: the first E2E attempt failed because `ivps list`'s per-node probe timed out under launch load, and `cloudify_runbook_bind_targets` (`lib/runbooks.sh:1027`) does not check `_cloudify_target_resolve`'s status - the resolver's `die` is swallowed (it runs inside `$(...) || return 1`, where errexit is ignored), so an empty triple reached the bindings file and the manifest was refused by the schema with a confusing message, leaving a temp file behind. The bug is pre-existing, not caused by the rename; the re-run with a healthy inventory was green.

### 2026-09-14 - CRITICAL GATE replaced by the fragile-surface rule

- Rachid's call: the gate was too costly. Its objective - awareness of the two fragile mechanisms before editing - is now met by `docs/FRAGILE.md` (invariant -> lives at -> pinning test) plus `task gate` (142 invariant tests: payload and registry goldens, walker claim order, declaration gate, context lifecycle, the three shadow suites), triggered by a 12-line AGENTS.md section listing exactly which files are fragile.
- Consent only for contract changes (visit order, formats, ownership) or edits to a pinning test. Everything else in the repo is plain TDD. AGENTS.md shrank from 104 to 93 lines; the detail moved to `docs/FRAGILE.md`, where it can change with the code at no per-session token cost.
- Session math that drove it: 7 gate cycles, ~2,000 lines of artifacts, 2 touching the fragile surface.

### 2026-09-14 - resolver failure no longer swallowed into a bindings row (plain TDD)

- First found by the two-host E2E during the vocabulary slice: when `ivps list`'s per-node probe timed out, `_cloudify_target_resolve` died correctly, but `cloudify_runbook_bind_targets` (`lib/runbooks.sh:1027`) never checked its status - the function runs inside the caller's `$(...) || return 1`, where bash ignores errexit, so the die printed and an empty triple became a bindings row. The manifest was then refused by the schema with a confusing message and a temp manifest was left behind. Proven in isolation with a three-function repro mirroring the real call chain.
- Fix: `resolved=$(_cloudify_target_resolve "$addr") || return 1` (the resolver's own message is what the operator sees, nothing is written), and `cloudify_manifest_validate_file` now prints the reason and returns 1 instead of exiting, so `_cloudify_manifest_render_write` removes its temp file before aborting. Ran as plain TDD under the new fragile-surface rule: red test (`bind: a resolver failure fails the bind`), two-line fix, `task gate` + lint + full unit 658 ok / 0 not ok (`results/errexit/report.tap`), two-host E2E 4 green with 4 later-phase scenarios skipped by name (`results/errexit-e2e2/report.tap`).
- Flake recorded: the E2E is sensitive to `ivps list` probe timeouts under launch load (2 of 5 runs today hit it, one before and one after the fix). Every runbook step is a fresh `cloudify` process that re-probes the live inventory, so a busy node can fail a step mid-run. Not fixed here; a candidate is a short TTL on the inventory cache across the step's child processes.

### 2026-09-14 - plan and REDESIGN realigned to the fragile-surface rule
