...
- Registry distribution (experiment, URGENT per Rachid 2026-09-10): the registry lives on the operator host only, a single point of failure and a barrier to multi-operator fleets. Experiment: publish/replicate registry events (NATS/JetStream or equivalent) so records survive an operator loss and several operators can read them. Open question: operator-local append + async replication vs a central broker as the write master.
