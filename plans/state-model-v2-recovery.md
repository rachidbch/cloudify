...
- [ ] Prove every `manifest.json` is actual JSON that validates against `schemas/v1/deployment-manifest.schema.json` before atomic replacement. (The atomic ordering already holds: render to a temp under the lock, validate, `mv`. The gap is that the schema check is skipped when `jq` is absent (`_cloudify_manifest_reference_check` returns 1, accepted at `lib/state.sh:416-419`), so "validates against the schema" is not guaranteed. The mandatory-checker edit under R2.2 closes it.)
...
- [ ] Prove lifecycle ordering: `applying` before mutation, `active` after install plus verify, `degraded` after observed failure. (`applying` and `degraded` already hold (`lib/runbooks.sh:1273-1275`, `:1590-1591`). `active` is false: the rule checks only `grep -qx install` (`lib/runbooks.sh:1594-1596`), so a `--phase install` run flips a half-run to `active`; fix it and add a `--phase install` test.)
...
- [ ] Keep only `runbooks/<application>/<flavor>/runbook.md` discovery. (Gap: `cloudify_runbook_find` still globs every `*.md` and matches on front-matter `deployment:` (`lib/runbooks.sh:966`), so any Markdown under the runbooks root is discoverable, against `REDESIGN.md`.)
...
- [ ] Delete dead aliases, duplicate parsers, duplicate path builders and stale compatibility language. (Concrete set: `add_in_hosts` (`lib/utils.sh:398`) with zero callers; the public `cloudify_vars_set/delete/list/show` and `_cloudify_deployment_read_vars` (`lib/vars.sh:722-726`) referenced only by tests; the undocumented `migration` alias (`cloudify:651`); the stale `_cloudify_note_bg` comment (`cloudify:194-195`); the stale value-source list in `README.md:198`, `:201`.)
- [ ] Reduce `lib/runbooks.sh`, `lib/state.sh`, `lib/context.sh`, `lib/deployments.sh` and `lib/vars.sh` where the same fact is parsed or formatted more than once; inline each duplicate into one existing parser rather than adding another abstraction layer. (Concrete: one pure `.remote-vars` enumerator with `cloudify_vars_pkg_read` keeping the only claim and provenance write (the six copies are `lib/vars.sh:349-368`, `:698-712`, `lib/context.sh:100-125`, `lib/registry.sh:238-255`, `lib/runbooks.sh:159-177`, `:1093-1113`); the front-matter reader (`lib/runbooks.sh:254`, `:561`, `:598`); the `--target`/`--phase` argv checks (four and five spellings in `lib/runbooks.sh`); the bare-run phase set (`:122` versus `:413`); one JSON escaper.)
- [ ] Replace `lib/state.sh`'s hand-rolled JSON encoder, field reader and bindings parser (`_cloudify_manifest_render`, `_cloudify_manifest_field_file`, `_cloudify_manifest_parse_bindings`) and its optional-jq shell fallback with `jq`, so the JSON artifacts (the deployment manifest, run records, package state) share one encoder and one validator. (`jq` becomes a hard prerequisite of the state path: a loud pre-mutation failure when it is absent, and a README prerequisite line. Absent today: `lib/state.sh:357-368` returns "unusable", accepted at `:416-419`.)
- [ ] Replace the all-zeros `_CLOUDIFY_MANIFEST_NULL_COMMIT` sentinel with real `null` in the manifest and run records, now that the schemas allow it, and make the shell manifest validator accept null only with `development_override: true`. (Schema already allows it; the sentinel is written at `lib/runbooks.sh:796` and defined at `lib/state.sh:65`; no test pins it. Renderer, writer guard and validator change in one slice, or a `CLOUDIFY_DEVELOPMENT_OVERRIDE=1` run writes a manifest its own validator rejects.)
- [ ] Fix `cloudify deployments`: it prints every first-level directory under `CLOUDIFY_DEPLOYMENTS_DIR` (`lib/deployments.sh:93-99`), which is now application names, because that root holds both the nested `<app>/<flavor>/<name>` desired-input tree and the legacy single-ID run directories. Accept only the nested shape that carries `values.yaml`, with a test.
