...
- Registry distribution moved to non-urgent (durability SPOF; backups must be secret-aware, never git). Target-inventory adapter seam filed non-urgent. Consent for the aligned model pending.
...
- E2E smoke: all three forms (`cloudai:cloudify`, `:cloudify` with `CLOUDIFY_NODE`, bare `cloudify`) resolve to the same container and the same ssh target; the ssh hop is blocked by the workstation's tailnet policy (the `ssh` rules lost `dst: tag:incus`), so the smoke was completed over an incus-exec transport substitute and the ACL restore is proposed pending consent.
- Blocker for the real ssh path: `ivps acl revoke <dst> --ssh` strips `$dst` from EVERY `ssh` rule (`_acl_transform_revoke` has no src/ip match on the ssh branch), so today's revoke removed `tag:incus` from the ssh rules that predate branch 6 (snapshot `acl-20260822-204345` has it) and left the lighthouse rule with `dst: []`. cloudify integration tests (`ssh root@cloudify`) cannot run until that is restored.
