...
#### `.remote-vars` — var names in repo, values from caller env (ADR-007)

Packages that need per-deployment values (tokens, URLs) declare the var NAMES in
the repo; the caller supplies the VALUES via environment at install time.
`pkg/<name>/.remote-vars` lists one name per line (`#` comments allowed):

```
# pkg/k3s-server/.remote-vars
K3S_TOKEN
```

```bash
K3S_TOKEN=abc123 cloudify --on host install k3s-server
```
