...
**Status:** Accepted
**Context:** The Phase 0 spike (2026-07-31) ran the k3s multi-cluster design against the live tailnet — 2 throwaway incus nodes registered as `tag:k3s-spike`, k3s v1.33.3+k3s1 server + agent. The cluster reached both-nodes-Ready with working cross-node pod traffic (ping 3/3, 8MB TCP in 3s). After teardown the ACL was restored to its pristine state (incl. comments), nodes deleted, authkey revoked. This ADR records the corrections and recipe requirements the spike forced; it supersedes the grant-syntax and operator-reach wording of ADR-009 (whose per-cluster-tag isolation intent holds — validated green).
**Decision:**
1. **ACL tag lifecycle is 3-phase, not one POST.** A grant referencing a tag HTTP-400s "tag not found" until a node carries that tag. Correct order: (a) declare the tag in `tagOwners` (POST-able with no node present — verified 200); (b) mint a tagged authkey + register a node under the tag (the tag now exists); (c) add the `ssh.dst` entry + grants. F1 (`ivps tag create`) must implement this order idempotently.
